      ...
      try {
        const [s, t] = await Promise.all([
          fetch(`/api/pons/v2/${token}/summary/`).then(async (r) => {
            if (!r.ok) throw new Error(r.status === 404 ? "This token was not launched through Peach Pad." : "Could not load this token right now.");
            return (await r.json()) as SummaryResponse;
          }),
          fetch(`/api/pons/v2/${token}/trades/`).then(async (r) =>
            r.ok ? ((await r.json()) as TradesResponse) : { trades: [] },
          ),
        ]);
        if (cancelled) return;
        setSummary(s);
        ...
        if (!cancelled) {
          setError(e instanceof Error ? e.message : "Could not load this token right now.");
          clearInterval(id);
        }
      }
    };
    ...
    <Shell>
      ...
      >
        <section className="space-y-4">
          <div className="pixel-panel px-5 py-6 sm:px-7">
            ...
            <div className="mt-6 grid grid-cols-2 gap-3 border-t-4 border-ink/10 pt-5 sm:grid-cols-4">
              ...
              <Stat label="MCap" value={formatCap(m.marketCapUsd, m.marketCapEth)} />
          ...
          <div className="pixel-panel px-5 py-5">
            ...
            {trades && trades.trades.length > 0 ? (
              ...
              <p className="text-xs text-ink/50">
                {m.graduated ? `Graduated — trading continues on ${site.firstPad}.` : "No trades yet."}
              </p>
            )}
          </div>
        </section>
        ...
        <aside className="space-y-4">
          ...
          <CreatorFees token={m.token} fees={fees} />
        </aside>
      </motion.div>
    </Shell>
  );
...
function ChartPanel({
  ...
  const m = summary.market;
  const usd = chart?.ethUsd ?? null;
  const series = useMemo(() => {
    if (!chart || chart.points.length === 0) return null;
    const supply = chart.launchSupply;
    return chart.points.map((p) => ({ t: p.t, value: p.price * supply * (usd ?? 1) }));
  }, [chart, usd]);
    ...
    <div className="border-4 border-ink bg-white shadow-[4px_4px_0_0_#1c1410]">
      <div className="flex flex-wrap items-end justify-between gap-3 border-b-4 border-ink px-4 py-3">
        <div>
          ...
          <p className="font-display text-2xl text-ink">{formatCap(m.marketCapUsd, m.marketCapEth)}</p>
          {change !== null ? (
        ...
        <p className="ui-text text-[11px] text-ink/40">via {site.firstPad} curve</p>
      </div>
      {series ? (
        <MarketCapChart points={series} format={usd === null ? formatEthAuto : formatUsdCompact} />
      ) : (
...
function CreatorFees({ token, fees }: { token: string; fees: SummaryResponse["fees"] }) {
  const { isConnected, chainId } = useAccount();
  const { writeContractAsync, isPending } = useWriteContract();
  const [status, setStatus] = useState<string | null>(null);

  const pending = BigInt(fees.pendingWei || "0");
  const toCreator = (pending * BigInt(site.creatorShareBps)) / 10000n;
  const toPad = pending - toCreator;
  const canCollect = ROUTER_ADDRESS !== null && isConnected && chainId === robinhoodChain.id && pending > 0n;
  ...
  return (
    <div className="pixel-panel px-5 py-6">
      ...
      <p className="mt-2 font-display text-2xl text-peach">{formatWeiExact(pending)} ETH</p>
      ...
      <p className="mt-3 text-[11px] text-ink/45">
        Accruing on the curve: {formatWeiExact(fees.accruingWei)} ETH · swept to escrow by {site.firstPad}
      </p>
      ...
      <p className="mt-4 text-[11px] leading-relaxed text-ink/45">
        Anyone can trigger a collect. Connect a wallet to send it.
      </p>
